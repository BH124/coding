<!-- this is signup_page.php for student -->
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="p-8 max-w-md w-full bg-white rounded shadow">
    ...
    <form action="signup_page.php" method="post">
      ...
      <div class="mb-6">
        ...
        <input id="password" name="password" type="text" placeholder="Enter your password" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-indigo-500">
      </div>
      <div class="mb-6">
        <label for="password" class="block text-gray-700 text-sm font-bold mb-2">Confirm Password</label>
        <input name="confirm_password" id="confirm_password" type="text" placeholder="Confirm your password" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-indigo-500">
      </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['button_regist'])) {
        ...
        // Button 2 is clicked, redirect to signin_page.php
        header("Location: signin_page.php");
        exit(); // Make sure to exit after redirecting
    }
}
